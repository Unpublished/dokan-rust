...
fn main() -> Result<(), Box<dyn std::error::Error>> {
	...
	let mount_point = U16CString::from_str(matches.get_one::<&str>("mount_point").unwrap())?;

	let mut flags = MountFlags::ALT_STREAM;
	if matches.contains_id("dokan_debug") {
		flags |= MountFlags::DEBUG | MountFlags::STDERR;
	}
	if matches.contains_id("removable") {
		flags |= MountFlags::REMOVABLE;
	}

	let options = MountOptions {
		single_thread: matches.contains_id("single_thread"),
		flags,
		..Default::default()
	};
